...
const UniqueIdFactoryContext = React.createContext<UniqueIdFactory | undefined>(
  undefined,
);

export const UniqueIdProvider = ({children}: {children: React.ReactNode}) => {
  const factory = React.useMemo(() => new UniqueIdFactory(), []);
  return (
    <UniqueIdFactoryContext.Provider value={factory}>
      {children}
...
export function useUniqueId(
  prefix = '',
  idOverride: string | undefined = undefined,
) {
  const idFactory = React.useContext(UniqueIdFactoryContext);
  ...
  const uniqueIdRef = React.useRef<string | null>(idOverride ? idOverride : null);
