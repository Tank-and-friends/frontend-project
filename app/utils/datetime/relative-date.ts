...
import {DateRangeValue, RelativeDateValue} from '../../types';
...
export function formatDateRange(value: DateRangeValue, format = DATE_FORMAT) {
  if (value.relative) {
    ...
    return `${relativeLabel} (${formatDate(start, format)} - ${formatDate(
      end,
      format,
    )})`;
  }
  ...
  return capitalize(
    [
      start ? `từ ${formatDate(start, format)}` : undefined,
      end ? `đến ${formatDate(end, format)}` : undefined,
    ]
      .filter(Boolean)
      .join(' '),
  );
...
export function formatDateRangeShort(
  ...
  options?: {
    ...
    format?: string;
    hideRelativeDate?: boolean;
  },
) {
  const {
    format = DATE_FORMAT,
    hideRelative = false,
    hideRelativeDate = false,
  } = options || {};
  ...
  return capitalize(
    [
      start ? formatDate(start, format) : undefined,
      end ? formatDate(end, format) : undefined,
    ]
      .filter(Boolean)
      .join(' - '),
  );
...
export function getRelativeRange(relative: RelativeDateValue): {
  start: Date;
  end: Date;
} {
  const now = new Date();
